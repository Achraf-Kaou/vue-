    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <div class="px-6 py-4">
        <h3 class="text-black font-medium flex items-center">
          <i class="fa fa-language mr-3 text-xl"></i>
          Activity Tester
        </h3>
      </div>
      ...
      <!-- Content -->
      <div class="p-6">
        <div v-if="activities && Object.keys(activities).length > 0">
          <div class="mb-4 text-sm text-gray-500">
            <i class="fas fa-info-circle mr-1"></i>
            Displaying {{ Object.keys(activities).length }} activities that were successfully launched
          </div>
          ...
          <div class="flex justify-end mb-4">
            <button
              @click="viewMode = 'grid'"
              :class="['px-3 py-2 rounded-l-md border border-gray-300', viewMode === 'grid' ? 'bg-green-100 text-green-800 border-green-300' : 'bg-white text-gray-700']"
            >
              <i class="fas fa-th mr-1"></i> Grid
            </button>
            <button
              @click="viewMode = 'table'"
              :class="['px-3 py-2 rounded-r-md border border-gray-300 border-l-0', viewMode === 'table' ? 'bg-green-100 text-green-800 border-green-300' : 'bg-white text-gray-700']"
            >
          ...
          <div v-if="viewMode === 'grid'" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <div v-for="(desc, img) in activities" :key="img" class="bg-gray-50 rounded-lg overflow-hidden shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
              <div class="relative pb-[180%] overflow-hidden bg-gray-100">
                ...
                <div class="absolute bottom-0 right-0 m-2">
                  <button
                    @click="openLightbox(img)"
                    class="bg-white text-gray-800 p-1 rounded-full shadow-md hover:bg-green-50"
              ...
              <div class="p-4">
                <div class="font-medium text-gray-900 mb-1 line-clamp-2" :title="desc">{{ desc }}</div>
          ...
          <div v-else class="overflow-x-auto">
            <table class="w-full">
              ...
              <tbody class="divide-y divide-gray-200">
                <tr v-for="(desc, img) in activities" :key="img" class="hover:bg-gray-50 transition-colors">
                  <td class="px-6 py-4">
                    <div class="relative w-32 h-56 mx-auto">
                      ...
                      <button
                        @click="openLightbox(img)"
                        class="absolute bottom-2 right-2 bg-white text-gray-800 p-1 rounded-full shadow-md hover:bg-green-50"
                        title="View full size"
                      >
                        <i class="fas fa-search-plus"></i>
                      </button>
        ...
        <div v-else class="text-center py-12">
          ...
          <p class="text-gray-500 max-w-md mx-auto">
            The analysis did not identify any activities that could be launched successfully.
          </p>
        </div>
        ...
        <div v-if="lightbox.visible" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75" @click="closeLightbox">
          <div class="max-w-4xl max-h-screen p-4">
            <div class="relative">
              ...
              <div class="absolute bottom-4 left-0 right-0 text-center text-white bg-black bg-opacity-50 py-2 px-4">
                {{ lightbox.img ? activities[lightbox.img] : '' }}
              </div>
            </div>
          </div>
        </div>
